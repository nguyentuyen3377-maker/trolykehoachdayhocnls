...
const App: React.FC = () => {
  ...
  const [grade, setGrade] = useState(GRADES[2]); // Default Lớp 3
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setPlanRows(result.map((r: any) => ({ ...r, id: Math.random().toString(36).substr(2, 9), note: "" })));
    } catch (error) {
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm mb-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          ...
          <button 
            ...
            disabled={isLoading}
            className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-400 text-white rounded-xl py-3.5 font-bold transition shadow-lg shadow-indigo-100 flex items-center justify-center space-x-2"
          >
            {isLoading ? (
              <span className="flex items-center"><svg className="animate-spin h-5 w-5 mr-3" viewBox="0 0 24 24"></svg> Đang thiết lập...</span>
            ) : (
              <>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-[11px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-200">
                <th className="px-6 py-4 border-r border-slate-200 w-24">Tuần, Tháng</th>
                <th className="px-6 py-4 border-r border-slate-200 min-w-[200px]">Chủ đề/Mạch nội dung</th>
                <th className="px-6 py-4 border-r border-slate-200 min-w-[250px]">Tên bài học</th>
                <th className="px-6 py-4 border-r border-slate-200 w-20 text-center">Số tiết</th>
                <th className="px-6 py-4 border-r border-slate-200 min-w-[150px]">Địa chỉ tích hợp NLS (Mã chỉ báo)</th>
                <th className="px-6 py-4 border-r border-slate-200 min-w-[300px]">Gợi ý nội dung lồng ghép/điều chỉnh</th>
              ...
              {planRows.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-20 text-center text-slate-400 italic">
                    Nhấn "Thiết lập bằng AI" để tự động tạo kế hoạch 35 tuần...
                  </td>
                </tr>
              ) : (
                planRows.map((row) => (
